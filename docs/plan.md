Implementation plan · v3.1 · 2026-09-05 · second play session folded in: the hand's keys, the answer line, one callout, the mouse
...
| **M Mouse** | every screen: `MouseArea`/`TapHandler` on every control with a hover state; the picker's cards and the rival tags as targets; the garage's steppers and swatches; settings rows; results buttons | the Lode Runner plugin's mouse handling, and the garage's own keyboard flow as the parity bar | a harness run that drives every screen by synthetic clicks alone, and one by keys alone, reaching the same states; hover visible; no control reachable by one and not the other; `omarchy.emojis`-style dismiss on the scrim unchanged | M5″ |
...
- **Digits are only ever the answer.** No key that can be a digit may choose, arm, confirm or spend a card. The hand is Left, Right, Up, Down and Space, and nothing else (design v4.1).
- **The mouse is not optional.** A control that works by key and not by click is a defect on any screen, from piece M onward.
...
v3.1 order: **F first**, under the v4.1 rubric (the hand's keys, the one answer line, one callout, the stall on the field, the victim's impact, the Pile-Up's 900 ms telegraph); then **M**, because it touches every screen and the picker F is still shaping, so it cannot run beside F; then **T**, which shares `TrackView.qml` with both. **Rotated to T → F → M on 2026-09-05, by the coordinator, for the reason the rule above gives.** When v3.1 was written, a piece T builder was already several hundred lines into `TrackView.qml`, `shaders/road.frag`, `ui/CanvasRoad.qml` and `ui/parts/SunsetSky.qml`. F round 7 and M both edit `TrackView.qml`, `Race.qml` and `Picker.qml`, so starting either while T held that file would have put two builders in one file — the hazard this tree has already paid for once, and the hazard the last sentence of this paragraph forbids. Nothing is dropped and no piece changes its contents; the order is rotated so that no two agents ever share a file. T lands and commits, then F round 7 carries the whole v4.1 rubric, then M. **The listed order is a default; "nothing runs in parallel with a piece that edits the same file" outranks it.**
...
> Continue the Turbo Tables Solo build under `docs/plan.md` v3.1 and `docs/design.md` v4.1. Read `docs/open-questions.md` §5 first: it is the maintainer's second play session, and every item in it must be addressed in the pieces it names before those pieces can be won. Pieces 1, 2, 6 (v1), C and 7 are frozen; do not re-run them. The prop kit under `assets/props/` is frozen art: place it, animate its frames, never redraw it; smoke, sparks and the tow line are drawn in QML. Run piece F to a win under the v4.1 rubric — digits never touch the hand, Space fires, the fact and answer are one line, one callout slot, the stall on the field, impact on the victim, the Pile-Up telegraph at 900 ms — then piece M (the mouse on every control), then piece T (the shader world and the kit placed by sector), then 3 and 5 under v4.1 including §5.4's words, then 6, then 7 in the VM. Never two pieces on one file at once. For each piece fan out a builder and a fresh critic; the critic compares ours against the bar blind, labels stripped, on that piece's rubric, and is harsh — the picture and the feel have to win, not the interface. Every Qt process is headless; never open a window on this Mac. Never revert with git in this tree, and never `git add -A`; stage the paths you changed. Never break the rules under "Rules the builder may not break." `/loop` on each piece until the critic picks ours. When every open piece is won and CI is green, or when the VM is unreachable at piece 7, write `HANDOFF.md` as the plan describes and stop.